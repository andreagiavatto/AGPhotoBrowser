...
@property (nonatomic, strong, readwrite) UILabel *pageIndexLabel;
...
- (void)layoutSubviews
{
    ...
    self.pageIndexLabel.frame = CGRectMake((CGRectGetWidth(self.frame)-80)/2, CGRectGetHeight(self.frame) - 32, 80, 32);
...
- (void)setupView
{
    ...
    [self.sharingView addSubview:self.pageIndexLabel];
...
- (void)setPageindex:(NSString *)pageindex
{
    ...
    if ([_pageindex length]) {
        self.pageIndexLabel.text = _pageindex;
    } else {
        self.pageIndexLabel.text = @"";
    }
...
{
    if (!_pageIndexLabel) {
        _pageIndexLabel = [[UILabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.frame)-80)/2, CGRectGetHeight(self.frame) - 32, 80, 32)];
        _pageIndexLabel.textColor = [UIColor colorWithWhite:0.9 alpha:0.9];
        _pageIndexLabel.font = [UIFont systemFontOfSize:14];
